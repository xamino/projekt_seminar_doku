...
For reasons explained in \ref{sinusoidalApprox} we made do with the exactness of the first two eigenpostures for jumping jack and rope skipping. Hence, in the following we will use equation \label{eq:approxPosture} with n=2.
...
% Todo: Wie kommt man von der PCA auf die Coeffizienten für Sinusdarstellung (mit Formel)

\subsection{Sinusoidal--Approximation of Coefficients}\label{sinusoidalApprox}
A graphical representation of the coefficients of the first two \emph{eigenpostures} of a jumping jack motion can be seen at the top of figure \ref{fig:approx}.
Here the blue curve represents the first coefficient of the \emph{eigenposture} and the green curve the second coefficient as functions of the time.
...
The approximation is done using MATLAB's integrated \emph{fit} function, which finds for each \emph{eigenposture} the amplitude, the frequency and phase so that the sine function defined by these parameters fits best to the corresponding coefficients.
...
Where all $\phi_{i,j}$ are with respect to $\phi_{1,1}$ and $\omega$ is $\omega_{1,1}$, all other parameters as they were described before.
...
Especially the coefficients of the second eigenposture sometimes were hard to discribe using the sum of two sine functions. Therefore we used a third approach to describe a jumping jack motion.
...
With rope skipping, as seen in figure \ref{fig:approx3}, the only solid way to descibe the coefficients of either eigenposture was a single sine function.
...
Therefore a very general set of parameters - two amplitudes $a_{1}$, $a_{2}$, two frequencies $\omega_{1}$, $\omega_{2}$ and one phase $\phi$ with respect to the other phase - was used to describe the coefficients.
...
\begin{figure}
	...
	\caption{Centering effect on coefficients. The top left graph shows the sinusoidal approximation a recorded motion without any centering.
	Here the second eigenvector's coefficients show a linear noncyclic behavior over the length of the recording.
...
\hline
\multirow{3}{*}{Jumping Jack} & 1 per eigenvalue & 18 & \multirow{3}{*}{19} \\ \cline{2-3}
 & 2 for 1. eigenvalue, 1 for 2. eigenvalue  & 17 \\ \cline{2-3}
 & 2 per eigenvalue & 13 \\ \cline{2-3}
\hline
Rope Skipping & 1 sine per eigenvalue & 16 & 16 \\
\end{tabular}
\end{center}
