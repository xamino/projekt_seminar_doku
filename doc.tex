...
We experienced that the frequency of the second eigenposture's coefficients is twice the frequency of the first eigenposture's coefficients.
Therefore we only need to know the first frequency.
Since the time when the recording started, is irrelevant, we don't need to know the first phase, but we need to know the second phase with respect to the first one.
...
At the time t, an approximated posture p(t) can now be reconstructed using (See \cite{origin}):

\begin{align}
	p(t) = p_{0} + p_{1} \cdot a_{1} \cdot sin(\omega t) + p_{2} \cdot a_{2} \cdot sin(2 \omega t + \phi) \label{eq:approxPostureSin}
\end{align}

Where $\omega$ is the frequency of the first sine function, $\phi$ is the phase of the second sine function with respect to the phase of the first one, and $a_{i}$ is the amplitude of the i-th sine function.
